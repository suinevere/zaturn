...
Output: game_patched/zork1_engflag_fw/
"""
...
OUTDIR = os.path.join(ROOT, "game_patched", "zork1_engflag_fw")
OUT_T1 = os.path.join(OUTDIR, "Zork1 (engflag fw) (Track 01).bin")
OUT_CUE = os.path.join(OUTDIR, "Zork1 (engflag fw).cue")
...
MSG_PATCH = {36: ("half", "west of house")}
LOOSE_PATCH = {0x06042d44: ("fw", "You are standing in an open field, west of a white house. Score: 0.")}
...
def build_patched_0zork(orig):
    ...
    def put(mode, text):
        nonlocal cur
        b = enc_fw(text) if mode == "fw" else enc_half_lower(text)
