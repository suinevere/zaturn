...
/*----------------------
 ...
 |   already (via menu_fade_out or menu_intro_fade_arm) and drawn its first
 |   frame, or there is nothing to reveal.
 | Author: suinevere
 | Dependencies: SRL, display.c (display_bg_rgb)
 | Globals: g_display
 | Params: frames -- ramp length in fields
...
void menu_fade_in(int frames) {
    for (int i = 0; i <= frames; i++) {
        menu_intro_level(-255 + (255 * i) / frames);
        SRL::Core::Synchronize();
    }
...
/*----------------------
 | menu_fade_clear
 | Description: See menu.h. Instantly (no ramp) reveals a held-black screen:
 |   zeroes colour offset A, releases both channels, and restores the full
 |   backdrop colour. Used to cut straight into gameplay after the menu has
 |   faded to black and the story has loaded behind the black, and to make an
 |   error screen visible if a load fails mid-black.
 | Author: suinevere
 | Dependencies: SRL, display.c (display_bg_rgb)
 | Globals: g_display
 | Params: N/A
