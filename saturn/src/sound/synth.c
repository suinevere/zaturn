...
/*----------------------
 | SYNTH_WAVE_TABLE / SYNTH_NOISE_TABLE / synth_waves_build
 | Description: The four tonal waveform tables and the percussion waveform, and
 |   the call that fills them. Separate tables because they are different
 |   lengths. Not const, and not linked data: synth_waves.c builds both into RAM
 |   at boot, which costs the netbin image 5,120 fewer file bytes than carrying
 |   them in .rodata. tools/assets/genwaves.py is still the authority for what
 |   the numbers are, and saturn/tests/test_synth_waves.py compiles the builder
 |   on the host and diffs all 5,120 bytes against it.
 | Author: suinevere
 ----------------------*/
extern signed char SYNTH_WAVE_TABLE[4][SCSP_WAVE_MAX];
extern signed char SYNTH_NOISE_TABLE[SCSP_NOISE_LEN];
