...
static void opcode_save(void)
{
#if defined(MOJOZORK_SATURN)
    {
        ...
        /* Scratch comes from LWRAM, not the C heap: the heap is shared with the
           story image and the typeahead trie and cannot spare this much mid-game.
...
static void opcode_restore(void)
{
#if defined(MOJOZORK_SATURN)
    {
        ...
        /* LWRAM scratch, and a dynamic-memory-sized pristine copy -- see
           opcode_save. These allocations gate the device/slot picker below, so a
           failure here is what silently swallowed the whole restore UI. */
        uint8 *orig = (uint8 *) saturn_scratch_alloc(dyncap);
        uint8 *blob = (uint8 *) saturn_scratch_alloc(maxblob);
        ...
        if (!saturn_load_blob(blob, maxblob) || (memcmp(blob, "MZSV1", 5) != 0) ||
            !saturn_read_story_prefix(orig, storylen, dyncap)) {
            saturn_scratch_free(orig);
            saturn_scratch_free(blob);
