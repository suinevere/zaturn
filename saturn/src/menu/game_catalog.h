...
/*----------------------
 ...
 | Description: Forgets the cached catalogue so the next preload_game_catalog
 |   re-scans /Z3 from the root. Call on the soft reset: the scan is the only thing
 |   that re-establishes the Z3 directory record every later story open resolves
 |   against, and a cached catalogue skips it for the whole of the next session.
 | Author: suinevere
 | Dependencies: N/A
