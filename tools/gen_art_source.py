...
"""/*----------------------
 ...
 | Author: suinevere
 | Dependencies: argparse, hashlib, json, pathlib, sys, forge_client,
 |     scene_vocab
...
def draw(entry, seed, host, steps=None, cfg=None):
    """/*----------------------
     | draw
     | Description: One plate: composed over a reference image when the room
     |     names one, drawn from the words alone when it does not.
     | Author: suinevere
     | Dependencies: forge_client
     | Globals: REFS
     | Params: entry -- a batch entry; seed -- the seed; host -- the server;
     |     steps, cfg -- overrides
     | Returns: PNG bytes
     ----------------------*/"""
    common = dict(host=host, negative=negative_for(entry),
                  checkpoint=entry.get("checkpoint"),
                  steps=steps if steps is not None else forge_client.STEPS,
                  cfg=cfg if cfg is not None else forge_client.CFG)
    ref = entry.get("compose_from")
    if not ref:
        return forge_client.txt2img(entry["prompt"], seed, **common)
    path = REFS / ref
    if not path.is_file():
        raise SystemExit(f"gen_art_source: {entry['name']} composes from "
                         f"{path}, which is not there")
    return forge_client.img2img(entry["prompt"], seed, path.read_bytes(),
                                denoise=float(entry.get("denoise", 0.6)),
                                **common)
...
def fingerprint(entry):
    """/*----------------------
     | fingerprint
     | Description: What a plate was drawn FROM, short enough to keep beside
     |     it in the manifest.
     |
     |     Recorded because a plate on disk says nothing about whether it is
     |     current. Prompts changed under a running job repeatedly and the old
     |     plates sat there looking finished, and were reported as fresh
     |     failures twice. Everything that decides the picture goes in: the
     |     prompt, what it refuses, the reference it is composed over and how
     |     hard, and the checkpoint.
     | Author: suinevere
     | Dependencies: hashlib
     | Globals: N/A
     | Params: entry -- a batch entry
     | Returns: a 16-character hex digest
     ----------------------*/"""
    parts = [entry["prompt"], entry.get("negative", ""),
             entry.get("compose_from", ""), str(entry.get("denoise", "")),
             entry.get("checkpoint", "")]
    return hashlib.sha256("".join(parts).encode("utf-8")).hexdigest()[:16]


def negative_for(entry):
    """/*----------------------
     | negative_for
     | Description: The negative prompt for one plate: the whole-disc one, plus
     |     whatever that room alone has to refuse.
     | Author: suinevere
     | Dependencies: forge_client
     | Globals: N/A
     | Params: entry -- a batch entry
     ...
     ----------------------*/"""
    extra = entry.get("negative")
    if not extra:
        return forge_client.NEGATIVE
    return forge_client.NEGATIVE + ", " + extra
...
def redraw(args, plates, man):
    ...
    for name in args.redraw:
        ...
        (ART / f"{name}.png").write_bytes(
            draw(e, seed, args.host, args.steps, args.cfg))
...
def main(argv=None):
    ...
    ap.add_argument("--checkpoint", default=forge_client.CHECKPOINT,
                    help="the checkpoint to draw with, loaded once at the "
                         "start and left loaded")
    ...
    if args.checkpoint and ckpt and not ckpt.startswith(args.checkpoint):
        have = forge_client.models(args.host)
        if have and args.checkpoint not in have:
            raise SystemExit(
                f"gen_art_source: the server has no checkpoint "
                f"{args.checkpoint!r}. It has: {', '.join(have)}")
        print(f"loading {args.checkpoint} (was {ckpt})", flush=True)
        forge_client.set_checkpoint(args.checkpoint, args.host)
        ckpt = forge_client.alive(args.host)
    ...
    for n, e in enumerate(todo, 1):
        ...
        png.write_bytes(draw(e, e["seed"], args.host, args.steps, args.cfg))
        man["plates"].append({
            "source": png.name,
