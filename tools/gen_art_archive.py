...
"""/*----------------------
 ...
 |     committed, the archives are derived, and the frame offsets have to be in
 |     a committed table because the runtime reaches a frame by offset alone. So
 |     the offsets are written down here, next to a SHA-256 of the archive they
 |     were measured in, and a rebuild that produces different bytes says so
 |     instead of leaving the table pointing into the middle of a record. That
 |     rebuild has to be reproducible for this to hold: the encoder has no
 |     randomness, and stylise's grain is drawn from a seeded generator for the
 |     same reason.
 |
 |     The archives are committed too, under saturn/cd/data/BG, even though they
 |     are derived. Nothing that could rebuild them is available where the disc
 |     is published: styling grades a plate against the ORIGINAL frame beside it,
 |     so a rebuild needs bg.bat's staged archives -- which come out of a disc a
 |     release workflow has no business downloading -- as well as Pillow, numpy
 |     and ~25 minutes on four workers. The SHA-256 beside each offset is what
 |     makes committing them safe rather than merely convenient: it is the same
 |     record either way, and CI checks the built disc against it.
 |
...
def main(argv=None):
    ...
    out = {
        "_comment": "GENERATED by tools/gen_art_archive.py -- where each "
                    "generated picture lies inside its archive, and the size "
                    "and SHA-256 of the archive each offset was measured in. "
                    "The archives are committed under saturn/cd/data/BG; this "
                    "is what holds them to the offsets, and what lets "
                    "gen_presentation.py extend IMAGE_FRAME without opening "
                    "them.",
        "prefix": PREFIX,
        "cap": args.cap,
