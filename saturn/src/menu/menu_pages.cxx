...
/*----------------------
 ...
 |   or the on-screen keyboard grid under pad control, plus explicit Ok and
 |   Cancel rows below the grid. Seeds the edit buffer from g_dialnum, then
 |   loops: a real-keyboard char/backspace/enter/escape/clear edits or
 |   accepts/cancels directly; otherwise the pad moves a cursor that starts
 |   in the on-screen keyboard grid (Up/Down/Left/Right highlight a key, C
 |   types it) and, from the grid's bottom row, Down carries it onto Ok then
 |   Cancel (Left/Right also toggle between the two once there, Up climbs
 |   back into the grid) -- `arow` tracks this: -1 in the grid, 0 on Ok, 1 on
 |   Cancel. B backspaces; A always accepts and Start always cancels
 |   regardless of cursor position, matching the shortcuts every other
 |   Options page hint advertises. B and Backspace cancel instead of
 |   backspacing when the buffer is already empty, so clearing the field and
 |   backing out is one continuous run of the same button rather than a dead
 |   end that forces a reach for Start/Esc. Accept validates the buffer with
 ...
 |   the Ok/Cancel rows every frame.
 | Author: suinevere
...
static void network_page(void) {
    ...
    const char *err = "";
    int arow = -1;   // -1 = cursor in the KB grid; 0 = Ok; 1 = Cancel
    ...
    for (;;) {
        ...
        if      (ke.kind == SATURN_KEY_CHAR)      { if (k.input_len < DIALNUM_MAX) keyboard_type_char(&k, ke.ch); }
            ...
            if (g_pad->WasPressed(Button::Up)) {
                if      (arow == 1) arow = 0;
                else if (arow == 0) arow = -1;
                else                keyboard_move(&k, 0, -1);
            }
            if (g_pad->WasPressed(Button::Down)) {
...
/*----------------------
 ...
 | Author: suinevere
 | Dependencies: music.c (music_cdda_audio_tracks/music_resume/
...
void sound_options_page(void) {
    ...
    // Reached from the in-game Options menu, the music is paused; this page is the
    // one place that cannot work in silence, since every row on it is judged by ear.
    // A no-op everywhere else. Remembered rather than assumed, because the way back
    // out has to put the drive in the state this page found it: the in-game Options
    // menu underneath is silent and stays that way until saturn_glue's read loop
    // resumes it (saturn_glue.cxx:266), while the main menu's music was never
    // stopped and must not be. See the restore after the loop.
    ...
    for (;;) {
        ...
        bool ok   = g_pad->WasPressed(Button::A) || g_pad->WasPressed(Button::C)
                  || ke.kind == SATURN_KEY_ENTER;
        bool cancel = g_pad->WasPressed(Button::B) || g_pad->WasPressed(Button::START)
                    || ke.kind == SATURN_KEY_ESCAPE || ke.kind == SATURN_KEY_BACKSPACE;
    ...
    // not after: the root Options menu this is leaving to is silent, so the fade
    // should be silent too. Ok's music_start()/music_refresh() has already run by
    // now, so this holds the track it just picked; music_cdda_pause falls back to
    // replaying that track whole if it catches the drive still seeking, which is
    // the documented and better failure.
...
/*----------------------
 ...
 |   Cancel (or B/Esc/Start) discards the local copy, leaving g_difficulty
...
static void gameplay_page(void) {
    ...
    for (;;) {
        ...
        menu_clear();
        int fx, fy, fw, fh;
        menu_box_fit("GAMEPLAY", 34, 9, &fx, &fy, &fw, &fh);
        ...
        y += 3;
        if (nums) text_print(x, y++, "%c 2) Ok", sel == GR_OK ? '>' : ' ');
        else      text_print(x, y++, "%c    Ok", sel == GR_OK ? '>' : ' ');
        if (nums) text_print(x, y++, "%c 3) Cancel", sel == GR_CANCEL ? '>' : ' ');
...
/*----------------------
 | options_menu
 | Description: Options menu (centered box): Save Game, Load Game, and Return
 |   to Title (shown only while a game is in progress), Gameplay, Display,
 |   Sound (shown only when there is audio to configure), Controls, Network,
 |   and Done. Builds a dynamic item list -- Save Game/Load Game lead when
 |   g_in_game is set; Gameplay, Display, Controls, and Network are always
 ...
 |   screen; Done always follows. Credits lives on the title mode-select menu
 ...
 |   Game and Load Game close the menu immediately like Done, but report
 ...
 |   sized via menu_box_fit from the actual item count (4..7 rows, depending
...
int options_menu(void) {
    ...
    for (;;) {
        ...
        ay += 2;
        text_print(x0 + 2, ay, "%s", hint("A/C=Ok  B=Back", "Enter=Ok  B=Back"));
        menu_sync();
        if (need_fade_in) { page_fade_in(g_menu_page_fade); need_fade_in = false; }
    }
    page_fade_out(g_menu_page_fade);
