...
/*----------------------
 | CATEGORY_DIR / CATEGORY_BAND
 | Description: The disc folder each text category's pictures live in, and
 |   (via CATEGORY_BAND, below) where its genre bands sit inside the folder's
 |   1..99 index range. The folder name is the only name a picture has now --
 |   files inside it are a bare two-digit index -- so a slot is
 |   (category, index) rather than a position in a scanned list.
 |
 ...
 |
 |   CATEGORY_BAND is generated by tools/make_tga.py from the files that
 |   actually converted, so a count can never name a picture the disc lacks.
 | Author: suinevere
 ----------------------*/
...
/*----------------------
 ...
 |   opening on the same one. Holds an absolute 0-based index into the
 |   category's 1..99 range, not an offset within a band -- see
 |   display_set_art_band and display_next_in_band.
 | Author: suinevere
 ----------------------*/
...
/*----------------------
 ...
 | Description: See display.h. Re-seats every category's rotor to its new
 |   effective band's base on an actual change, because a rotor left over
 |   from the old band validates against category_total and would otherwise
 |   surface a picture from the wrong band, silently, until the next rotate
 |   or shuffle happened to land it back in range. A no-op when band matches
 |   the current one -- Task 6 calls this ahead of every rotation on every
 |   room change, and re-seating unconditionally would reset an in-progress
 |   rotation on each call.
 | Author: suinevere
 | Dependencies: N/A
 | Globals: g_art_band, g_cat_rot, CATEGORY_BAND (via effective_band)
...
/*----------------------
 ...
 | Author: suinevere
 | Dependencies: N/A
 | Globals: CATEGORY_BAND (via display_slot_make -> category_total)
