...
/*----------------------
 ...
 |   Cancel. B backspaces; A and Start both accept regardless of cursor
 |   position, matching the shortcuts every other Options page hint advertises.
 |   B and Backspace cancel instead of backspacing when the buffer is already
 |   empty, so clearing the field and backing out is one continuous run of the
 |   same button. Accept validates the buffer with valid_dialnum before
 |   committing it into g_dialnum and calling options_save().
 |
 ...
 |   saving and without a warning, since discarding is what was asked. Redraws the
...
static void network_page(void) {
    ...
    for (;;) {
        ...
        y++;
        text_print(x, y, "%s",
            hint("C=type B=del  A/Start=Ok", "type number  Enter/Esc=Ok"));
        menu_sync();
