...
> **Partly stale.** Everything below about the prompt sheet, the plate counts,
> the sampler settings and what is on disk was overtaken on 2026-09-04 by
> [[art-prompt-and-model-handoff]] -- the checkpoint, the guidance, the step
> count and every prompt have changed since, and the 725 drawn plates it
> reports no longer exist. What remains true is the archive work: the CGL
> packer, the depth-first record ordering, the byte-wide index widened to a
> short, and the per-record-read measurement that was reverted.
