...
Branch `controller-command-interface2`, `b47b2fb..ba370b4` (eleven commits).
Continues the work [[command-panel-and-dim-handoff]] describes; that entry's
"never been compiled or run" is now **stale** — see the note at its head.

`git log --oneline b47b2fb..ba370b4` is the list and each message states its own
reason. What follows is only what the messages and the code cannot say.

## What the owner has actually seen

The owner ran builds during this session. Confirmed working on screen:

- **`8d58184`** — panel Recall, and the cursor holding still under a chord shift.
- **`6ef6a5b`** — the black display border.
...
is what prompted `ba370b4`. **Do not read that as sign-off on the rest** —
...
2. **Gameplay keeps nothing.** The Interface row moved out of it entirely
   (`f7aff19`); Controls is now the only editor for `g_cmd_iface`.
...
- **`code-review`** with base `b47b2fb` if the owner wants these reviewed before
  merge — `menu_pages.cxx` and `command_view.cxx` carry most of the risk.
