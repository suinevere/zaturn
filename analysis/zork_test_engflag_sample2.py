...
OUTDIR = os.path.join(ROOT, "game_patched", "zork1_engflag_sample2")
OUT_T1 = os.path.join(OUTDIR, "Zork1 (engflag sample2) (Track 01).bin")
OUT_CUE = os.path.join(OUTDIR, "Zork1 (engflag sample2).cue")
...
def enc(s):
    ...
    for ch in s:
        if ch == " ":
            out.append(0x60)                 # -> glyph 0x7F (blank), control-safe
        elif ch == "\n":
            out.append(0x0c)
        else:
