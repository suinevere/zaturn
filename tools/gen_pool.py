...
"""/*----------------------
 ...
 |     plus any pictures generated since, which together are the art supply
 |     every game on the disc draws from.
 |
 ...
 | Author: suinevere
 | Dependencies: art_frames, csv, json, pathlib, re, sys
...
def generated_images(idx_area):
    """/*----------------------
     | generated_images
     | Description: The pictures that were never on the original disc, as pool
     |     rows in the same shape the measured ones take. Their rooms and tracks
     |     are empty and stay empty: no Zork I room ever showed one, and an
     |     invented precedent is exactly the kind of thing the whole scene table
     |     exists to stop. What makes one choosable instead is its own account of
     |     what it shows, which image_looks answers for measured and generated
     |     pictures alike.
     |
     |     Cross-checked against IMAGE_FRAME rather than trusted: frames.json
     |     says which archive a picture landed in and the table says which
     |     archive each index belongs to, and the two disagreeing means the
     |     table was generated against a different build of the art.
     | Author: suinevere
     | Dependencies: art_frames
     | Globals: N/A
     | Params: idx_area -- index -> area stem, from the generated table
     | Returns: a list of pool image rows
     ----------------------*/"""
    out = []
    for f in art_frames.frames():
        i = int(f["index"])
        if idx_area.get(i) != f["archive"]:
            raise SystemExit(
                f"gen_pool: {f['source']} says it is picture {i} in "
                f"{f['archive']}, but IMAGE_FRAME puts picture {i} in "
                f"{idx_area.get(i, 'nothing')} -- re-run tools/gen_art_archive.py "
                "and tools/gen_presentation.py together")
        out.append({
            "index": i,
            "png": f["png"],
            "area": f["archive"],
...
def main():
    ...
    images += generated_images(idx_area)

    pool = {
        "_comment": "GENERATED by tools/gen_pool.py -- the picture and track "
                    "supply every game on the disc draws from: Zork I's own, "
                    "plus any generated pictures, which carry no rooms because "
                    "no room of Zork I ever showed one. index is the "
                    "IMAGE_FRAME index a room record stores.",
        "images": images,
        "tracks": track_table(),
